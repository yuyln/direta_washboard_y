module variaveis
    ...
    integer, parameter:: ni = 5000, ns = 1, np = 7, nc = 3000, &
&                        corte = 1, relax = 0
    integer:: nxs, nys, cxs, cys
    integer, dimension(:, :, :), allocatable:: caixa_skyr
    integer, dimension(:, :), allocatable:: ncaixa_skyr
    doubleprecision, parameter:: fator = 1.d-6
    doubleprecision, parameter:: tf = 500.d0, hs = tf / ni, &
    &                            pi = dacos(-1.d0), lx = 36.d0, &
    &                            ly = 36.d0, xhi = 1.d0, &
    &                            temp = 0.d0, fcf = 3.d0, &
    &                            ac = 0.d0, &
    &                            u = 1.d0, rp = 0.65d0, &
...
&                                beta = betadamp * damping
    ...
    doubleprecision:: fcx, fcy, aux, hx, hy, wy, hxs, hys
    logical:: esq, dir, cima, baixo, caixax, caixay
end module
...
subroutine atualiza_caixas
    ...
    do i = 1, nxs
        ...
        cont = 0
            do i_s_ = 1, ns
                caixax = (x0(i_s_) < (i * hxs)).and.(x0(i_s_) >= (i - 1) * hxs)
                caixay = (y0(i_s_) < (j * hys)).and.(y0(i_s_) >= (j - 1) * hys)
                if (caixax.and.caixay) then
                    cont = cont + 1
                    caixa_skyr(i, j, cont) = i_s_
                    ncaixa_skyr(i, j) = ncaixa_skyr(i, j) + 1
                endif
            enddo
        enddo
    enddo
...
subroutine forcas(desx, desy)
    ...
    ssx = 0.d0
    ssy = 0.d0
    cxs = nint((x0(i_s) + desx) / (lx / nxs)) + 1
    cys = nint((y0(i_s) + desy) / (ly / nys)) + 1
    if (cxs < 1) cxs = 1
    if (cys < 1) cys = 1
    if (cxs > nxs) cxs = nxs
    if (cys > nys) cys = nys
    do i___ = -1, 1
        ...
        do j = -1, 1
            ...
            if (cima) enty = 1
            if (baixo) enty = nys
            if (.not.cima.and..not.baixo.and..not.esq.and..not.dir) then
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
                    if ((dabs(x0(i__) - x0(i_s) - desx) <= bb).and.((dabs(y0(i__) - y0(i_s) - desy) <= bb))) then
                        if (i__ /= i_s) call skyr_skyr(x0(i_s) + desx, y0(i_s) + desy, x0(i__), y0(i__), ssx, ssy, tab_bessel, xhi)
                    endif
                enddo
            endif
            ...
            if (cima) then
                ...
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
                    if ((dabs(x0(i__) - x0(i_s) - desx) <= bb).and.((dabs(y0(i__) + ly - y0(i_s) - desy) <= bb))) then
                        if (i__ /= i_s) call skyr_skyr(x0(i_s) + desx, y0(i_s) + desx, x0(i__), y0(i__) + ly, ssx, ssy, &
                        &tab_bessel, xhi)
                    endif
                enddo
            endif
            ...
            if (baixo) then
                ...
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
                    if ((dabs(x0(i__) - x0(i_s) - desx) <= bb).and.((dabs(y0(i__) - ly - y0(i_s) - desy) <= bb))) then
                        if (i__ /= i_s) call skyr_skyr(x0(i_s) + desx, y0(i_s) + desy, x0(i__), y0(i__) - ly, ssx, ssy, &
                        &tab_bessel, xhi)
                    endif
                enddo
            end if
            ...
            if (esq) then
                ...
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
                    if ((dabs(x0(i__) - lx - x0(i_s) - desx) <= bb).and.((dabs(y0(i__) - y0(i_s) - desy) <= bb))) then
                        if (i__ /= i_s) call skyr_skyr(x0(i_s) + desx, y0(i_s) + desy, x0(i__) - lx, y0(i__), ssx, ssy, &
                        &tab_bessel, xhi)
                    endif
                enddo
            endif
            ...
            if (esq.and.cima) then
                enty = 1
                entx = nxs
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
                    if ((dabs(x0(i__) - lx - x0(i_s) - desx) <= bb).and.((dabs(y0(i__) + ly - y0(i_s) - desy) <= bb))) then
                        if (i__ /= i_s) call skyr_skyr(x0(i_s) + desx, y0(i_s) + desy, x0(i__) - lx, y0(i__) + ly, ssx, &
                        &ssy, tab_bessel, xhi)
                    endif
                enddo
            endif
            
            if (esq.and.baixo) then
                enty = nys
                entx = nxs
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
            ...
            if (dir.and.cima) then
                enty = 1
                entx = 1
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
                    if ((dabs(x0(i__) + lx - x0(i_s) - desx) <= bb).and.((dabs(y0(i__) + ly - y0(i_s) - desy) <= bb))) then
                        if (i__ /= i_s) call skyr_skyr(x0(i_s) + desx, y0(i_s) + desy, x0(i__) + lx, y0(i__) + ly, &
                        &ssx, ssy, tab_bessel, xhi)
                    endif
                enddo
            endif
            
            if (dir.and.baixo) then
                enty = nys
                entx = 1
                do i_ = 1, ncaixa_skyr(entx, enty)
                    i__ = caixa_skyr(entx, enty, i_)
...
program principal
    ...
    use gera_function
    call cortes
    allocate(caixa_skyr(1: nxs, 1: nys, 1: ns))
    allocate(ncaixa_skyr(1: nxs, 1: nys))
    call ler_skyr(ns, x0, y0, lx, ly)
    call g_bessel(xhi)
    call g_wash(ly, np, u)
    call atualiza_caixas
    open(11, file='dados.dat', status='unknown')
    write(11, "(a20, 2f10.5)") "Range Bessel: ", ab, bb
    write(11, "(a20, 2f10.5)") "LX, LY: ", lx, ly
    write(11, "(a20, 2i10)") "NXS, NYS: ", nxs, nys
    write(11, "(a20, 2f10.5)") "HXS, HYS: ", hxs, hys
...
10      format(f6.2, "%", f7.3)
    ...
    call cpu_time(fim)
    print*, fim - inicio
    deallocate(caixa_skyr)
    deallocate(ncaixa_skyr)
end program
